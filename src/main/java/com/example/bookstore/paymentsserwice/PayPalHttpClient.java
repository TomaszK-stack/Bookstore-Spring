...
import com.example.bookstore.paymentsDTO.AccessTokenResponseDTO;
import com.example.bookstore.paymentsDTO.ClientTokenDTO;
import com.example.bookstore.paymentsDTO.OrderDTO;
import com.example.bookstore.paymentsDTO.OrderResponseDTO;
...
import static com.example.bookstore.paymentsserwice.PayPalEndpoints.createUrl;
...
public class PayPalHttpClient {
    ...
    public AccessTokenResponseDTO getAccessToken() throws Exception {
        var request = HttpRequest.newBuilder()
                .uri(URI.create(createUrl(paypalConfig.getBaseUrl(), PayPalEndpoints.GET_ACCESS_TOKEN)))
    ...
    public ClientTokenDTO getClientToken() throws Exception {
        ...
        var request = HttpRequest.newBuilder()
                .uri(URI.create(createUrl(paypalConfig.getBaseUrl(), PayPalEndpoints.GET_CLIENT_TOKEN)))
    ...
    public OrderResponseDTO createOrder(OrderDTO orderDTO) throws Exception {
        ...
        var request = HttpRequest.newBuilder()
                .uri(URI.create(createUrl(paypalConfig.getBaseUrl(), PayPalEndpoints.ORDER_CHECKOUT)))
                .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessTokenDto.getAccessToken())
